...
float get_beta(int iter, int adapt, float beta_init, int T)
{
    if(adapt==1){
	return iter*1.0/(iter+3);		
    }
    if(adapt==2){
        return beta_init * (1-iter*1.0/T)/(1-beta_init+beta_init*(1-iter*1.0/T));
    }
    else return 0;
}
        ...
        *fval = *rval = *upper = 0;
        for(int i=1; i<=m; i++) {
            ...
            *fval += zi / div;
            ...
            //*rval += prob->clause_weight[i]*(clause_sat[i] != 0);
            *rval += (clause_sat[i] != 0);
...
void do_maxsat(SATProblem *prob, Model *model, Parameter *param)
{
    ...
    static int best_rval = 0;


    int m = prob->m, n = prob->n;
    int total_weight = 0;
    for(int i=0;i<m;i++) total_weight+=prob->clause_weight[i];
    ...
    for(int i=1; i<=m; i++) {
        dzero(g, k);
        daxpy(g, -1., V[0], k);
        daxpy(Z[i], 1.*prob->clause_weight[i], g, k);
    }
    for(int j=1; j<=n; j++) {
        int *p = index[j];
        for(; *p != 0; p++) {
            double sign = (*p > 0) ? 1. : -1;
            int i = abs(*p);
            daxpy(Z[i], prob->clause_weight[i]*sign, V[j], k);
        }
    }
    ...
    printf("iter %3d  fval %.14e  delta %.4e  rval %d/%d  %.4e  upper %.4e  time %.14e\n",
            0, fval, 0., rval, total_weight, rval*1./m, upper/m, 0.);
    }
    fval_prev = fval;
    ...
    for(; iter < max_iter; iter++) {
        for(int j=1; j<=n; j++) {
            ...
            for(p = index[j]; *p != 0; p++) {
                double sign = (*p>0) ? 1. : -1.;
                int i = abs(*p);
                daxpy(Z[i], -sign*prob->clause_weight[i], V[j], k);
            }
            // Do Mixing
            dzero(g, k);
            for(p = index[j]; *p; p++) {
                double sign = (*p>0) ? 1. : -1.;
                int i = abs(*p);
                daxpy(g, sign/((clen[i]+1)*clen[i]+2)*prob->clause_weight[i], Z[i], k);
            }
            ...
            // Adding momentum
            if(param->adapt!=0){
                beta = get_beta(iter,param->adapt,param->beta,param->max_iter);
                //printf("beta=%f\n",beta);
            }
           // printf("beta=%f\n",beta);
            daxpy(g,beta,V[j],k);
        ...
        if  (rval>best_rval){ 
            best_rval =  rval;
            if(param->expmode){
                printf("o %d time %.5e fval %.14e\n ",total_weight - best_rval, wall_time_diff(time_eval_ed-time_eval,time_st),fval);
            }
       }
        else if ((iter%100)==0){
                printf("time %.5e fval %.14e\n ",total_weight - best_rval, wall_time_diff(time_eval_ed-time_eval,time_st),fval);
        }
        fflush(stdout);
        //if(delta < eps)
         //   break;
...
//evaluate fval
         ...
         for(int i=0; i<n; i++) {
         for(const Node *p=C[i]; p->index != -1; p++) {
            newfval += p->value * ddot(V[i], V[p->index], k);
            newfval -= p->value;
            }
        }
        delta = fval-newfval;
